...
static int wilc_enqueue_cmd(struct message_queue *mq, struct host_if_msg *msg);
static int wilc_process_cmd(struct message_queue *mq, struct host_if_msg *msg);
...
static int wilc_mq_destroy(struct message_queue *mq)
{
	...
	while (!list_empty(&mq->msg_list)) {
		msg = list_first_entry(&mq->msg_list, struct host_if_msg, list);
		list_del(&msg->list);
		kfree(msg);
	}
...
static int wilc_process_cmd(struct message_queue *mq, struct host_if_msg *msg)
{
	...
	/* check buffer size */
	m = list_first_entry(&mq->msg_list, struct host_if_msg, list);
...
static void ListenTimerCB(unsigned long arg)
{
	...
	result = wilc_enqueue_cmd(&hif_msg_q, &msg);
	if (result)
...
static void host_if_work(struct work_struct *work)
{
	...
	/*
	TODO: check the significance of the send and recv
	vif = msg->vif;
	if ((!wilc_initialized)) {
		usleep_range(200 * 1000, 200 * 1000);
		wilc_enqueue_cmd(&hif_msg_q, &msg);
	}
	...
	*/
	...
	wilc_process_cmd(&hif_msg_q, msg);
	vif = msg->vif;
...
int wilc_init(struct net_device *dev, struct host_if_drv **hif_drv_handler)
{
	...
	*hif_drv_handler = hif_drv;
	...
	if (clients_count == 0)	{
		...
		hif_msg_q.wilc = wilc;
		hif_workqueue = alloc_workqueue("WILC_wq", WQ_MEM_RECLAIM, 0);
		if (!hif_workqueue) {
...
int wilc_set_tx_power(struct wilc_vif *vif, u8 tx_power)
{
	...
	ret = wilc_enqueue_cmd(&hif_msg_q, &msg);
	if (ret)
		netdev_err(vif->ndev, "wilc_enqueue_cmd fail\n");
