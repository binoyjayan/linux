...
static int init_cc_resources(struct platform_device *plat_dev)
{
	...
	new_drvdata->plat_dev = plat_dev;
		...
		set_dma_ops(&plat_dev->dev, &noncoherent_swiotlb_dma_ops);
	#else
		set_dma_ops(&plat_dev->dev, &coherent_swiotlb_dma_ops);
	#endif
#endif
...
static const struct of_device_id arm_cc7x_dev_of_match[] = {
	{.compatible = "arm,cc7x"},	// device id is taken from system.dts.dx in FPGA image
	{}
};
...
/* Module description */
MODULE_DESCRIPTION("ARM cc7xree Driver");
